use crate::chart::{
    ...
    indicator::{
        ...
        plot::{PlotTooltip, bar::{BarPlot, BarClass}},
    },
};
...
use exchange::{Kline, Trade, UnixMs, Volume, unit::Qty};
...
pub struct SessionDeltaWaveIndicator {
    cache: Caches,
    delta: BasisSeries<(UnixMs, Qty)>,
    data: BasisSeries<CumulativeDeltaPoint>,
    availability: IndicatorAvailability,
}

impl SessionDeltaWaveIndicator {
    pub fn new() -> Self {
        Self {
    ...
    fn indicator_elem<'a>(
        ...
        if let Some(message) = self.unavailable_message(main_chart, "Session Delta Wave") {
        ...
        let bar_kind = |_point: &CumulativeDeltaPoint| BarClass::Signed;
    ...
    fn rebuild_cumulative(&mut self) {
        ...
        self.data = self.delta.map(|&(time, delta)| {
    ...
    fn rebuild_from_deltas(&mut self, deltas: BasisSeries<(UnixMs, Qty)>) {
        self.delta = deltas;
        self.rebuild_cumulative();
    }
}

impl KlineIndicatorImpl for SessionDeltaWaveIndicator {
        ...
        let deltas = source.map_basis_series(
            |timeseries| {
                timeseries
                    .datapoints
                    .iter()
                    .map(|(&time, dp)| (time, (time, Self::datapoint_delta(dp))))
                    .collect()
            },
            |tickseries| {
                tickseries
                    ...
                    .map(|(idx, dp)| {
                        (
                            idx as u64,
                            (dp.kline.time, Self::delta_from_parts(&dp.footprint, dp.kline.volume)),
                        )
                    })
                    .collect()
            },
    ...
    fn on_insert_klines(&mut self, klines: &[Kline], source: &PlotData<KlineDataPoint>) {
        ...
        let has_data = {
            ...
            for kline in klines {
                ...
                deltas.insert(kline.time, (kline.time, delta));
        ...
        match source {
            PlotData::TimeBased(timeseries) => {
                ...
                for time in touched_times {
                    if let Some(dp) = timeseries.datapoints.get(&time) {
                        deltas.insert(time, (time, Self::datapoint_delta(dp)));
                        touched = true;
                    }
                }
            }
            PlotData::TickBased(tickseries) => {
                ...
                for (idx, dp) in tickseries.datapoints.iter().enumerate().skip(start_idx) {
                    ...
                    deltas.insert(idx as u64, (dp.kline.time, delta));
                    touched = true;
                }
            }
        }
